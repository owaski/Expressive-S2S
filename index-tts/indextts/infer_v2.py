...
class IndexTTS2:
    ...
    # 原始推理模式
    def infer(self, stress_control, spk_audio_prompt, text, output_path, # stress_control used
        ...
        if stream_return:
            return self.infer_generator(
                stress_control, spk_audio_prompt, text, output_path,
    ...
    def infer_generator(self, stress_control, spk_audio_prompt, text, output_path,
              emo_audio_prompt=None, emo_alpha=1.0,
              emo_vector=None,
              use_emo_text=False, emo_text=None, use_random=False, interval_silence=200,
        ...
        if self.cache_spk_cond is None or self.cache_spk_audio_prompt != spk_audio_prompt:
            ...
            semantic_code, S_ref = self.semantic_codec.quantize(spk_cond_emb)
            # print(f"===== DEBUG ===== S_ref shape: {S_ref.shape}")
            # print(f"===== DEBUG ===== spk_cond_emb shape: {spk_cond_emb.shape}")
            # print(f"===== DEBUG ===== the shape of semantic code is {semantic_code.shape}")
            # print(f"===== DEBUG ===== semantic code is {semantic_code}")
        ...
        # text_tokens_list = []
        if stress_control:
            stress_pos = get_stress_word_indices(text)
            normalized_text = remove_stress_control_markers(text)
            text_tokens_list = self.tokenizer.tokenize(normalized_text)
            text_tokens_list = insert_stress_tokens_preserving_positions(normalized_text, stress_pos, self.tokenizer)
            print(f"===== DEBUG =====, the stressed_text tokens list is  {text_tokens_list}")
        else:
            text = remove_stress_control_markers(text)
            text_tokens_list = self.tokenizer.tokenize(text)
            print(f"===== DEBUG =====, the normal text tokens list is {text_tokens_list}")
        # ========================================
        ...
        print(f"===== DEBUG =====, the text token ids are {text_token_ids}")
        ...
        for seg_idx, sent in enumerate(segments):
            ...
            with torch.no_grad():
                ...
                # print(f"===== DEBUG ===== the text token length in the below example is {torch.tensor([text_tokens.shape[-1]], device=text_tokens.device)}")
                # print(f"===== DEBUG ===== the code length in the below example is {torch.tensor([codes.shape[-1]], device=text_tokens.device)}")
                # print(f"===== DEBUG ===== the speech_conditioning_latent shape in the below example is {speech_conditioning_latent.shape}")
                # print(f"===== DEBUG ===== the spk_cond_emb shape in the below example is {spk_cond_emb.shape}")
                # print(f"===== DEBUG ===== the emo_cond_emb shape in the below example is {emo_cond_emb.shape}")
                with torch.amp.autocast(text_tokens.device.type, enabled=self.dtype is not None, dtype=self.dtype):
                    latent = self.gpt(
