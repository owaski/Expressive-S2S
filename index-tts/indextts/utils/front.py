...
class TextTokenizer:
    def __init__(self, vocab_file: str, normalizer: TextNormalizer = None):
        ...
        # Special stress control tokens mapping (outside vocab range)
        self.stress_token_to_id = {
            '<*>': self.vocab_size + 1,
            '</*>': self.vocab_size + 2
        }
        self.stress_id_to_token = {
            self.vocab_size + 1: '<*>',
            self.vocab_size + 2: '</*>'
        }
    ...
    def convert_ids_to_tokens(self, ids: Union[List[int], int]):
        # Handle special stress tokens that are outside vocab
        if isinstance(ids, int):
            if ids in self.stress_id_to_token:
                return self.stress_id_to_token[ids]
            return self.sp_model.IdToPiece(ids)
        else:
            tokens = []
            for id in ids:
                if id in self.stress_id_to_token:
                    tokens.append(self.stress_id_to_token[id])
                else:
                    tokens.append(self.sp_model.IdToPiece(id))
            return tokens
    ...
    def convert_tokens_to_ids(self, tokens: Union[List[str], str]) -> List[int]:
        ...
        ids = []
        for token in tokens:
            if token in self.stress_token_to_id:
                ids.append(self.stress_token_to_id[token])
            else:
