...
def de_tokenized_by_CJK_char(line: str, do_lower_case=False) -> str:
    ...
    do_lower_case:
      ...
      output = "see you!"
    """
    # ===== Modification ===== Check if the text contains CJK characters
    CJK_RANGE_PATTERN = r"[\u1100-\u11ff\u2e80-\ua4cf\ua840-\uD7AF\uF900-\uFAFF\uFE30-\uFE4F\uFF65-\uFFDC\U00020000-\U0002FFFF]"
    has_cjk = bool(re.search(CJK_RANGE_PATTERN, line))
    
    # If no CJK characters, just handle case conversion and return
    if not has_cjk:
        return line.lower() if do_lower_case else line
    # ========================
