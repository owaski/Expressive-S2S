...
class UnifiedVoice(nn.Module):
    def __init__(self, layers=8, model_dim=512, heads=8, max_text_tokens=120, max_mel_tokens=250, max_conditioning_inputs=1,
        ...
        # print(f"===== DEBUG ===== Text embedding module dimensions: {self.text_embedding.weight.shape}")
        ...
        self.speed_emb.weight.data.normal_(mean=0.0, std=0.0) # shape: (2, model_dim)
    ...
    def forward(self, speech_conditioning_latent, text_inputs, text_lengths, mel_codes, mel_codes_lengths, emo_speech_conditioning_latent,
        ...
        mel_codes:  long tensor, (b,m)
        mel_codes_lengths: long tensor, (b,)
...
def test():
    # init the UnifiedVoice model using the config file's params
    cfg = OmegaConf.load("/home/willw2/expressive_s2st/index-tts/finetune_config.yaml")
    model = UnifiedVoice(**cfg.gpt)
    print(model)
    # print(f"Number of trainable parameters: {calculate_num_params(model)}")
