...
class Stress17KPrecomputedDataset(Dataset):
    """
    Lightning-Compatible Stress17K Dataset that loads pre-extracted w2v-BERT features.
    Designed for stress-aware TTS training with pre-computed features.
    """
    
    def __init__(
        self,
        features_root: str = "/data/user_data/willw2/data/stress17k",
        metadata_path: str = "/data/user_data/willw2/data/stress17k_metadata_train_full_converted.json",
        split: str = "train_full_train",  # train_full_train/train_full_val
        ...
        # Initialize tokenizer (same as extraction script)
        ...
        print(f"===== STRESS17K DATASET ===== Vocab file: {self.tokenizer.vocab_file}")
        print(f"===== STRESS17K DATASET ===== Vocab size: {self.tokenizer.vocab_size}")
        ...
        # Filter by split
        self.metadata = [item for item in all_metadata if item.get("split") == split]
        print(f"Loaded {len(self.metadata)} samples for split '{split}' from metadata")
...
def collate_fn_stress17k_precomputed(batch: List[Dict]) -> Dict[str, torch.Tensor]:
    """
...
def create_stress17k_precomputed_dataloader(
    features_root: str = "/data/user_data/willw2/data/stress17k",
    metadata_path: str = "/data/user_data/willw2/data/stress17k_metadata_train_full_converted.json",
    batch_size: int = 8,
    num_workers: int = 4,
    split: str = "train_full_train",
    model_dir: str = "checkpoints",
    **dataset_kwargs
) -> DataLoader:
    """Create DataLoader for pre-computed Stress17K features"""
    
    dataset = Stress17KPrecomputedDataset(
    ...
    dataloader = DataLoader(
        ...
        shuffle=(split == "train_full_train"),
        num_workers=num_workers,
        collate_fn=collate_fn_stress17k_precomputed,
        pin_memory=True,
        drop_last=(split == "train_full_train")
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Test dataloader
        print("\nTesting DataLoader...")
        dataloader = create_stress17k_precomputed_dataloader(
            batch_size=2,
